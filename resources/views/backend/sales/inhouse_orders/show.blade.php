            ...
            <div class="card-body">
                                ...
                                {{-- old  form here --}}
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <input type="text" value="{{ $orderDetail->quantity }}" class="form-control" id ="quantity" name="quantity[]" required>
                                        </td>
                                        <td class="text-center">
                                        ...
                                        <td>
                                            <div class="prod_discount_cont">
                                                ...
                                                <?php
                                                 ...
                                                 ?>
                                                <input type="text" class="form-control" value="{{ $the_discount }}" name="discount[]">
                                            </div>
                                        </td>
                                        <td>
                                            <?php $pord_dis_type = \DB::table('products')->where('id', '=', $orderDetail->product_id)->select('discount_type')->get();
                                            //echo $pord_dis_type[0]->discount_type;
                                        ?>
                                            {{-- {{ $pord_dis_type}} --}}
                                            <select class="form-control" name="discount_type[]">
                                                <option value="none" @if($orderDetail->discount_type == 'none' || $orderDetail->discount_type == ''): selected @endif >None</option>
                                                <option value="amount" @if($orderDetail->discount_type == 'amount') selected @endif>Amount</option>
                                                <option value="percent" @if($orderDetail->discount_type == 'percent') selected @endif>Percent</option>
                                            </select>
                                        </td>
                                        <td class="text-center">
                                            {{ single_price($orderDetail->price) }}

                                            <input type="hidden" value="{{ ($orderDetail->price/$orderDetail->quantity) }}" name="prod_price[]">
                ...
                <div class="clearfix float-right">
                    <table class="table">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="shipping_cost" class="form-control" value="{{$delivery_charge->shipping_cost}}">
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="number" class="form-control" name="coupon_discount" value="{{ $order->coupon_discount }}">
                                </td>
                            </tr>
                            <tr>
                                <td>
                            ...
                            <tr>
                                <td colspan="2">
                                    ...
                                    <div class="form-group mb-0 text-right">
                                        <button type="submit" class="btn btn-block btn-primary">{{translate('Update')}}</button>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
                    ...
                    <div class="text-right no-print">
                        <a href="javascript:void(0)" type="button" class="btn btn-icon btn-light" onclick="print_a4_invoice('{{ route('admin.invoice.print_a4',$order->id) }}')"><i class="las la-print"></i></a>
                    </div>
                </div>
            </div>
        </form>
        				...
        				<thead>
            				<tr class="bg-trans-dark">
                                ...
                                <th class="text-uppercase">{{translate('Delivery Type')}}</th>
              					...
              					<th class="min-col text-center text-uppercase">{{translate('Price')}}</th>
              					<th class="min-col text-right text-uppercase">{{translate('Total')}}</th>
              					<th class="min-col text-right text-uppercase">{{translate('Update')}}</th>
            				</tr>
        				</thead>
        				<tbody>
                            @php
                            ...
                            @foreach ($order->orderDetails->where('seller_id', $admin_user_id) as $key => $orderDetail)
                             <form action="{{ route('all_orders.update_price', $order->id) }}" method="POST">
                                ...
                                <tr>
                                    <td>{{ $key+1 }}
                                    <input type="hidden" id ="orderDetailsID" name="orderDetailsID" value="{{ $orderDetail->id }}"></td>
                                    <td>
                                        @if ($orderDetail->product != null)
                                    ...
                                    <td>
                                     ...
                                     <td class="text-center"> <input type="text" value="{{ $orderDetail->quantity }}" id ="quantity" name="quantity"></td>
                                    <td class="text-center">
                                        {{ single_price($orderDetail->price/$orderDetail->quantity) }}
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <input type="hidden" value="{{ ($orderDetail->price) }}" name="old_price">
                                        <input type="hidden" value="{{($orderDetail->price/$orderDetail->quantity) }}" name="price">
                                        <input type="hidden" value="{{(round($order->orderDetails->sum('shipping_cost'))) }}" name="shipping_cost">
                                        <div class="form-group mb-0 text-right">
    			...
    			<table class="table">
          			<tbody>
                        ...
                        <tr>
            				...
            				<td>{{ single_price(round($order->orderDetails->sum('shipping_cost'))) }}</td>
            			</tr>
                        <tr>
            				<td>
