                ...
                <tbody>
                    @foreach ($products as $key => $product)
                        <tr>
                            ...
                            <td>
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input onchange="update_is_suggestion(this)" value="{{ $product->id }}"
                                        type="checkbox" <?php if ($product->is_suggestion == 1) {
                                            echo 'checked';
                                        } ?>>
                                    <span class="slider round"></span>
                                </label>
                            ...
                            <td class="text-right">
                                <a class="btn btn-soft-warning btn-icon btn-circle btn-sm" target="_blank"
                                    href="{{ route('products.barcode_print', $product->id) }}"
                                    title="{{ translate('Barcode Print') }}">
                                ...
                                <button type="button" class="btn btn-soft-dark btn-icon btn-circle btn-sm "
                            onclick="increaseProductStock('{{url('admin/inventory?')}}{{ 'product_id='.$product->id.'&button=adjust' }}')"
                                    title="{{ translate('Add Stock') }}">
...
@section('script')
 ...
 <script>
        function increaseProductStock(url) {
    ...
    <script type="text/javascript">
        ...
        function update_is_suggestion(el) {
            if (el.checked) {
                var status = 1;
            } else {
                var status = 0;
            }
            $.post('{{ route('products.is_suggestion') }}', {
                _token: '{{ csrf_token() }}',
                id: el.value,
                status: status
            }, function(data) {
                if (data == 1) {
                    AIZ.plugins.notify('success', '{{ translate('Suggestion product updated successfully') }}');
                } else {
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
            });
